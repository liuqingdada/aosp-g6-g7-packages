...
public class SecureElementSelector
    ...
    private boolean mSwp1Retry = false;
    private boolean mSwp2Retry = false;
    ...
    private void updateSeMapping() {
        ...
        if (mConfigFileParser.get(1, userConfig)) { /// SWP1
            try {
                ...
                if(!mSwp1Retry){
                    mSwp1Retry = true;
                    Log.d(TAG,"Set up the retry flag for SWP1!");
                }
                else{
                    mSwp1Retry = false;
                    Log.d(TAG,"It's already retried once for the SWP1.");
                }
            }
        }
        ...
        if (mConfigFileParser.get(2, userConfig)) { /// SWP2
            try {
                ...
                if(!mSwp2Retry){
                    mSwp2Retry = true;
                    Log.d(TAG,"Set up the retry flag for SWP2!");
                }
                else{
                    mSwp2Retry = false;
                    Log.d(TAG,"It's already retried once for the SWP2.");
                }
            }
        }
