...
public class FixLinearSnapHelper extends LinearSnapHelper {
    ...
    private int distanceToCenter(View targetView, OrientationHelper helper) {
        ...
        if ((helper.getDecoratedStart(targetView) == 0 && mRecyclerView.getChildAdapterPosition(
                ...
                && mRecyclerView.getChildAdapterPosition(targetView) == mRecyclerView.getAdapter()
                                                                                     .getItemCount() - 1)) {
            return 0;
        }

        int viewCenter = helper.getDecoratedStart(targetView) + (helper.getDecoratedEnd(
                targetView) - helper.getDecoratedStart(targetView)) / 2;
