...
//        }
    ...
    private void handleSmsSent(Intent intent, int error) {
        ...
        if (mResultCode == Activity.RESULT_OK) {
            ...
            if (sendNextMsg) { // this is the last part of a sms.a long sms's
                               // part is sent ordered.
                ...
                if (cursor != null) {
                    try {
                        if ((cursor.getCount() == 1) && cursor.moveToFirst()) {
                            ...
                            // so we shouldn't move it to other boxes.just keep
                            // it in failed box.
                            if (smsType != Sms.MESSAGE_TYPE_FAILED) {
                                ...
                                if (!Sms.moveMessageToFolder(this, uri, Sms.MESSAGE_TYPE_SENT,
                                        error)) {
                                    Log.e(TAG, "handleSmsSent: failed to move message " + uri
                                            + " to sent folder");
                                }
                            }
                        }
