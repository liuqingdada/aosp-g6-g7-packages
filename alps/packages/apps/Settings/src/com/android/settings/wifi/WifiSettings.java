...
public class WifiSettings extends RestrictedSettingsFragment
        ...
        switch (wifiState) {
            case WifiManager.WIFI_STATE_ENABLED:
                ...
                final Collection<AccessPoint> accessPoints =
                        constructAccessPoints(getActivity(), mWifiManager, mLastInfo, mLastNetworkInfo, mTm);
    ...
    private static List<AccessPoint> constructAccessPoints(Context context,
            WifiManager wifiManager, WifiInfo lastInfo, NetworkInfo lastNetworkInfo, TelephonyManager tm) {
        ...
        if (configs != null) {
            ...
            for (WifiConfiguration config : configs) {
                ...
                if (hasChangedSimCard(config, wifiManager, tm)) {
                    continue;
                }
                /// @}
    ...
    private void handleEvent(Intent intent) {
        ...
        Log.i(TAG, "handleEvent(), action = " + action);
    ...
    /* package */ void submit(WifiConfigController configController) {
        ...
        try {
            if (config != null && FeatureOption.MTK_EAP_SIM_AKA && config.imsi != null) {
                if (config.enterpriseConfig.getEapMethod() == WifiConfigController.WIFI_EAP_METHOD_SIM 
                		|| config.enterpriseConfig.getEapMethod() == WifiConfigController.WIFI_EAP_METHOD_AKA) {
            ...
            Log.d(TAG,"submit exception() " + e.toString());
        }
        /// @}
        if (config == null) {
            ...
            if (FeatureOption.MTK_EAP_SIM_AKA) {
                Log.d(TAG,"mSelectedAccessPoint " + mSelectedAccessPoint);
                List<WifiConfiguration> configs = mWifiManager.getConfiguredNetworks();
                if (configs != null) {
                    for (WifiConfiguration mConfig : configs) {
                        Log.d(TAG,"onClick() >>if ((mConfig.SSID).equals(mSelectedAccessPoint.ssid)) {");
                        Log.d(TAG,"onClick()" + mConfig.SSID);
                        Log.d(TAG,"onClick() " + mSelectedAccessPoint.ssid);
                        if (mConfig != null && mConfig.SSID != null && 
                            ...
                            Log.d(TAG, "remind user: cannot user eap-sim/aka under airplane mode");
                            Toast.makeText(getActivity(), R.string.eap_sim_aka_airplanemode, Toast.LENGTH_LONG).show();
                            return;
                        }
                    }
    ...
    public static final SearchIndexProvider SEARCH_INDEX_DATA_PROVIDER =
        new BaseSearchIndexProvider() {
            ...
            public List<SearchIndexableRaw> getRawDataToIndex(Context context, boolean enabled) {
                ...
                /// @}
                final Collection<AccessPoint> accessPoints =
                        constructAccessPoints(context, wifiManager, null, null, tm);
                for (AccessPoint accessPoint : accessPoints) {
         ...
         * @return
         */
        private static boolean hasChangedSimCard(WifiConfiguration config, WifiManager wifiManager, TelephonyManager tm) {
            ...
            if (FeatureOption.MTK_EAP_SIM_AKA && config.imsi != null && !config.imsi.equals("\"none\"")) {             
                ...
                if ((config.imsi).equals("\"1232010000000000@wlan.mnc001.mcc232.3gppnetwork.org\"")
                    ...
                    Log.d(TAG,"in simulator mode, skip");
                } else {
                    String imsiStr = null;
                    int[] SubIds = SubscriptionManager.getSubId(slot);
                    if (config.enterpriseConfig.getEapMethod() == Eap.SIM) {
                        imsiStr = WifiDialog.makeNAI(tm.getSimOperator(SubIds[0]), tm.getSubscriberId(SubIds[0]), "SIM");
                    } else if (config.enterpriseConfig.getEapMethod() == Eap.AKA) {
                        imsiStr = WifiDialog.makeNAI(tm.getSimOperator(SubIds[0]), tm.getSubscriberId(SubIds[0]), "AKA");
                    }
                    Log.d(TAG,"telephonyEx.getSubscriberId() " + tm.getSubscriberId(slot));
