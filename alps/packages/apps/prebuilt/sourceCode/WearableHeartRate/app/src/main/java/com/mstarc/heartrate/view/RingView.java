package com.mstarc.heartrate.view;
...
import com.mstarc.heartrate.heartrate.R;
...
public class RingView extends View {
     ...
     * 圆环 宽度
     * */
    private final int mHeartPaintWidth = -20;
     ...
     * 振幅
     * */
    private float AmplitudeA = 500;// 振幅
    ...
    private void init() {
        ...
        mHeartBeatPaint.setStrokeWidth(2);
        ...
        mHeartBeatPathPaint.setStrokeWidth(2);
        mHeartBeatPathPaint.setStyle(Style.STROKE);
        mHeartBeatPaint.setAntiAlias(true);
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mHeartBeatWidth = width;
        mFirstFrameOffset  =mHeartBeatWidth-1;
        AmplitudeA = mTotalHeight/2;
        ...
        // 周期定位总宽度的1/4
        mPeriodFraction = (float) (Math.PI * 2 / mHeartBeatWidth * 6);
        float periodFraction = (float) (Math.PI * 2 / mHeartBeatWidth * 12);
        for (int i =  mHeartBeatWidth/6; i < mHeartBeatWidth/6*2; i++) {
            mOriginalYPositon[i] = (float) (mTotalHeight/3 * Math.sin(periodFraction * i) + OFFSET_Y);
        }
...
//        //canvas.restoreToCount(level);
	...
	/*	if(StartHeartBeatAnmiFlag){
        ...
        if(StartFirstFrameFlag){
            for(int i=0,j=0;i<mHeartBeatWidth;i++,j++){
