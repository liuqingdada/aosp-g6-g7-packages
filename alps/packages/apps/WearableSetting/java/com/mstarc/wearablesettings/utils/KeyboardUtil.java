...
public class KeyboardUtil {
	...
	public boolean isSwitchChar = false;
	...
	public KeyboardUtil(Activity act, Context ctx, EditText edit) {
		...
		k2 = new Keyboard(ctx, R.xml.qwertyb);
		k3 = new Keyboard(ctx, R.xml.qwertyc);
	...
	private OnKeyboardActionListener listener = new OnKeyboardActionListener() {
		...
		public void onKey(int primaryCode, int[] keyCodes) {
			...
			} else if (primaryCode == 9527) {
				if(!isSwitchChar) {
					keyboardView.setKeyboard(k3);
				}else{
					keyboardView.setKeyboard(k2);
				}
				isSwitchChar = !isSwitchChar;
			} else {
		...
		if (isupper) {//大写切换小写
			...
			for(Key key:keylist){
				...
				if(key.codes[0] == Keyboard.KEYCODE_SHIFT) {
					key.icon = ctx.getDrawable(R.mipmap.bigsize);
				}
			}
		} else {//小写切换大写
			isupper = true;
			for(Key key:keylist){
				...
				if(key.codes[0] == Keyboard.KEYCODE_SHIFT) {
					key.icon = ctx.getDrawable(R.mipmap.smailsize);
				}
			}
		}
	}
