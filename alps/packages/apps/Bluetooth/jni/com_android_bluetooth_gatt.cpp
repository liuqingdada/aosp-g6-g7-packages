...
static void jstr2bdaddr(JNIEnv* env, bt_bdaddr_t *bda, jstring address)
{
    if (NULL == address)
    {
        ALOGE("[jstr2bdaddr] null address\n");
        return;
    };
    ...
    jboolean enable)
{
    ...
    if (NULL == address)
    {
        ALOGE("[gattClientRegisterForNotificationsNative] null address\n");
        return;
    };
    bt_bdaddr_t bd_addr;
        ...
        jbyteArray data, jbyteArray mask)
{
    switch(filt_type)
    {
        ...
        case 4: // BTM_BLE_PF_LOCAL_NAME
        {
            if (NULL == name)
            {
                ALOGE("[gattClientScanFilterAddRemoveNative] null name\n");
                return;
            };
                                 ...
                                 jint server_if, jstring address, jboolean is_direct, jint transport)
{
    ...
    if (NULL == address)
    {
        ALOGE("[gattServerConnectNative] null address\n");
        return;
    };
    bt_bdaddr_t bd_addr;
    const char *c_address = env->GetStringUTFChars(address, NULL);
