...
#include <hardware/bluetooth_mtk.h>
...
static const bt_interface_mtk_t *sBluetoothMtkInterface = NULL;
...
static bt_callbacks_mtk_t sBluetoothMtkCallbacks = {
	sizeof(sBluetoothMtkCallbacks),
...
static void classInitNative(JNIEnv* env, jclass clazz) {
    ...
    if (err == 0) {
        ...
        if (err == 0) {
            ALOGV("[BT HAL test] get default hal in\n");

            bluetooth_module_t* btStack = (bluetooth_module_t *)abstraction;
            sBluetoothInterface = btStack->get_bluetooth_interface();

            ALOGV("[BT HAL test] get default hal out %d\n", sBluetoothInterface);

            ALOGV("[BT HAL test] get mtk hal in\n");

            bluetooth_device_mtk_t* btMtkStack = (bluetooth_device_mtk_t *)abstraction;
            sBluetoothMtkInterface = btMtkStack->get_bluetooth_mtk_interface();
			ALOGV("[BT HAL test] get mtk hal out %d\n", sBluetoothMtkInterface);
...
static bool initNative(JNIEnv* env, jobject obj) {
	...
	if (sBluetoothMtkInterface) {
            ret = sBluetoothMtkInterface->init(&sBluetoothMtkCallbacks);
            if (ret != BT_STATUS_SUCCESS) {
                ALOGE("Error while setting Bluetooth mtk callbacks: %d\n", ret);
        ...
        if (ret != BT_STATUS_SUCCESS) {
            ...
            sBluetoothInterface->cleanup();
            sBluetoothInterface = NULL;
            sBluetoothMtkInterface->cleanup();
            sBluetoothMtkInterface = NULL;
            return JNI_FALSE;
        }
