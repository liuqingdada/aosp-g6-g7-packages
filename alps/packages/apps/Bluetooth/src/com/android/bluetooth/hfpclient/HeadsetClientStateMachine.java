...
final class HeadsetClientStateMachine extends StateMachine {
    ...
    static final int XPAL_REPORT = 55;
    static final int BATTERY_UPDATE = 56;
    ...
    private class Disconnected extends State {
        ...
        public void enter() {
            ...
            if (mRingtone != null && mRingtone.isPlaying()) {
                Log.d(TAG,"stopping ring after entering Disconnected Mode");
                mRingtone.stop();
            }
        ...
        public synchronized boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case DISCONNECT:
                    ...
                    break;
                case BATTERY_UPDATE:
                    updateBatteryLevel((Intent) message.obj);
                    break;
        ...
        private void processConnectionEvent(int state, int peer_feat, int chld_feat, BluetoothDevice device) {
            switch (state) {
                ...
                case HeadsetClientHalConstants.CONNECTION_STATE_SLC_CONNECTED:
                    ...
                    if (HeadsetClientHalConstants.HANDSFREECLIENT_IPHONE_BATTERY_STATUS_SUPPORTED) {
                        sendATCmdNative(HeadsetClientHalConstants.HANDSFREECLIENT_AT_CMD_IPHONE_XAPL,0, 0, null);
                        addQueuedAction(XPAL_REPORT);
                        // send battery status if it is ever updated
                        if (mBatteryLevel != -1) {
                            sendATCmdNative(HeadsetClientHalConstants.HANDSFREECLIENT_AT_CMD_IPHONE_BATTERY_STATUS,mBatteryLevel, 0, null);
                            addQueuedAction(BATTERY_UPDATE);
                        }
        ...
        public synchronized boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case QUERY_CURRENT_CALLS:
                    ...
                    break;
                case BATTERY_UPDATE:
                    if(updateBatteryLevel((Intent) message.obj)){
                        sendATCmdNative(HeadsetClientHalConstants.HANDSFREECLIENT_AT_CMD_IPHONE_BATTERY_STATUS,mBatteryLevel, 0, null);
                        addQueuedAction(BATTERY_UPDATE);
                    }
                    break;
        ...
        private void processAudioEvent(int state, BluetoothDevice device) {
            ...
            switch (state) {
                ...
                case HeadsetClientHalConstants.AUDIO_STATE_CONNECTED:
                    ...
                    /// M: call the setMode() after setParameters() @ {
                    /*
                    int newAudioMode = AudioManager.MODE_IN_CALL;
                    int currMode = mAudioManager.getMode();
                    if (currMode != newAudioMode) {
        ...
        public synchronized boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case DISCONNECT_AUDIO:
                    ...
                    if (disconnectAudioNative(getByteAddress(mCurrentDevice))) {
                        mAudioState = BluetoothHeadsetClient.STATE_AUDIO_DISCONNECTED;
                        /// M: do setMode() when DISCONNECT_AUDIO
                        mAudioManager.setMode(AudioManager.MODE_NORMAL);
                        //abandon audio focus
                        if (mAudioManager.getMode() != AudioManager.MODE_NORMAL) {
                                /// M: do setMode() when DISCONNECT_AUDIO
    ...
    boolean updateBatteryLevel(Intent intent)
    {
        ...
        batteryLevel = batteryLevel*10 / scale ;
        ...
        if (mBatteryLevel != batteryLevel) {
            ...
            Log.i(TAG, "Battery Level Changed :" + mBatteryLevel);
            return true;
        } else {
            Log.i(TAG, "Battery Level Same:" + mBatteryLevel);
            return false;
        }
    }
