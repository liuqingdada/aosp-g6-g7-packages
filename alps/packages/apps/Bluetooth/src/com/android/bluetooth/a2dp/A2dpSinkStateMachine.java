...
final class A2dpSinkStateMachine extends StateMachine {
    ...
    private static final String TAG = "[BT][A2DP]A2dpSinkStateMachine";
    ...
    private A2dpSinkStateMachine(A2dpSinkService svc, Context context) {
        super(TAG);//"A2dpSinkStateMachine");
        mService = svc;
        mContext = context;
    ...
    private BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context arg0, Intent intent) {
            ...
            if (action.equals("android.bluetooth.a2dp_sink.connect_confirm")) {
                ...
                Log.d(TAG, "[onReceive] n : " + n + ", address : " + address);
                if (n != -1) {
//                    confirmRequestNative(getByteAddress(address), n);
    ...
    private class Disconnected extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case REQUEST_CONFIRM:
                    ...
                    Log.d(TAG, "[Disconnected][processMessage] info.device : " + info.device 
                               + ", info.acceptOrReject : " + info.acceptOrReject);
                    if (!confirmRequestNative(getByteAddress(info.device), info.acceptOrReject)) {
                        Log.d(TAG, "[Disconnected][processMessage] confirmRequestNative failed");
                    ...
                    Log.d(TAG, "[Disconnected][processMessage] confirmRequestNative success");
                    if (info.acceptOrReject == CONFIRM_REQUEST_ACCEPT) {
        ...
        private void processConnectionEvent(int state, BluetoothDevice device) {
            Log.d(TAG, "[Disconnected][processConnectionEvent] state : " + state
                    + ", device : " + device);
            switch (state) {
    ...
    private class Pending extends State {
        ...
        private void processConnectionEvent(int state, BluetoothDevice device) {
            ...
            Log.d(TAG, "[Pending][processConnectionEvent] mCurrentDevice : " + mCurrentDevice
                    + ", mIncomingDevice : " + mIncomingDevice
                    + ", mTargetDevice : " + mTargetDevice);
            switch (state) {
                case CONNECTION_STATE_DISCONNECTED:
    ...
    private class Connected extends State {
        ...
        // in Connected state
        private void processConnectionEvent(int state, BluetoothDevice device) {
            Log.d(TAG, "[Connected][processConnectionEvent] state : " + state
                    + ", device : " + device);
            switch (state) {
                case CONNECTION_STATE_DISCONNECTED:
                    mAudioConfigs.remove(device);
        ...
        private void processAudioStateEvent(int state, BluetoothDevice device) {
            Log.d(TAG, "[Connected][processAudioStateEvent] state : " + state
                    + ", device : " + device);
            if (!mCurrentDevice.equals(device)) {
    ...
    private void controlA2dpSinkPlay(int action){
        ...
        if (mCurrentDevice != null){
            ...
            Log.e(TAG, "[controlA2dpSinkPlay] mCurrentDevice is NULL");
        }
    }
    ...
    private void broadcastConnectionState(BluetoothDevice device, int newState, int prevState) {
        ...
        int delay = mAudioManager.setBluetoothA2dpDeviceConnectionState(device, newState, BluetoothProfile.A2DP_SINK);

        Log.d(TAG, "[broadcastConnectionState] device : " + device + ", newState : " + newState
			+ ", prevState : " + prevState + ", delay " + delay);
        mWakeLock.acquire();
...
//FIXME            intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
            mContext.sendBroadcast(intent, ProfileService.BLUETOOTH_PERM);
            log("[onConnectionStateChanged] Connection state " + device + ": " + prevState + "->" + state);
