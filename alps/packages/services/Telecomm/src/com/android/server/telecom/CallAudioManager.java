...
final class CallAudioManager extends CallsManagerListenerBase
    ...
    private boolean isSvlteSupported() {
        return (SystemProperties.get("ro.mtk_svlte_support").equals("1"));
    }

    private int getPhone1Type() {
        int subIds[] = SubscriptionManager.getSubId(getExternalModemSlot());
        if (subIds == null) {
            Log.d(this, "getPhone1Type, subIds is null");
            return PhoneConstants.PHONE_TYPE_NONE;
        } else {
            Log.d(this, "getPhone1Type, subIds[0]:" + subIds[0]);
            return mTelephonyManager.getCurrentPhoneType(subIds[0]);
        }
    }

    private boolean isRoaming() {
        return isSvlteSupported() && getPhone1Type() == PhoneConstants.PHONE_TYPE_GSM;
    }

    private boolean isSvlteMultiSim() {
        return isSvlteSupported() && getPhoneCount() >= 2;
    }

    private boolean isDualtalk() {
        return (SystemProperties.get("ro.mtk_dt_support").equals("1"))
                || (isSvlteMultiSim() && !isRoaming());
    }
    ...
    private int getPhoneId(PhoneAccountHandle ahandle) {
        Log.d(this, "getPhoneId, ahandle:" + ahandle);
        if (ahandle == null) {
            Log.d(this, "getPhoneId: ahandle == null, return invalid");
            return SubscriptionManager.INVALID_PHONE_INDEX;
        }
        int subId = SubscriptionManager.INVALID_SUBSCRIPTION_ID;
        try {
            //M for EVDO world phone.@{
            String id = ahandle.getId();
            ...
            Log.d(this, "NumberFormatException, ahandle.getId():" + ahandle.getId());
        }
        return SubscriptionManager.getPhoneId(subId);
    }

    private int audioModeUpdateDualModem(int mode) {
        Log.d(this, "audioModeUpdateDualModem, mode:" + mode + ", phonecount:" + getPhoneCount());
        if (isDualtalk()) {
            Call call = getForegroundCall();
            ...
            if (call != null && call.isAlive()) {
                if (mode == AudioManager.MODE_IN_CALL) {
                    int phoneId = getPhoneId(call.getTargetPhoneAccount());
                    if (phoneId == SubscriptionManager.INVALID_PHONE_INDEX) {
                        Log.d(this, "At this point, we can't get the exactly phone id, only waiting the next time...");
                        return AudioManager.MODE_INVALID;
                    }

                    int newDualModemCall;
                    newDualModemCall = (phoneId == PhoneConstants.SIM_ID_2 ? 1 : 0);
                    if (mDualModemCall != newDualModemCall) {
        ...
        */
        ...
        if (isDualtalk() && getForegroundCall() != null && getForegroundCall().isAlive() &&
            ...
            if ((getFirstMD() == 2) &&
                (getExternalModemSlot() == -1)) {  //Dualtalk with two internal modems
        ...
        if ((isEvdoDualtalk() || isSvlteMultiSim())
            ...
            // if isEvdoDualtalk() is true but isDualtalk() is false, it means we are under GSM only mode (Gemini),
            // So we must map IN_CALL and IN_CALL_EXTERNAL to IN_CALL
            if (isDualtalk()) {

                if (getExternalModemSlot() == PhoneConstants.SIM_ID_1) {
                   newMode = (oldMode == AudioManager.MODE_IN_CALL) ? AudioManager.MODE_IN_CALL_EXTERNAL : AudioManager.MODE_IN_CALL;
        ...
        if (!isDualtalk()) {
            if (isEnableMD5() && (newMode == AudioManager.MODE_IN_CALL)) {
               newMode = AudioManager.MODE_IN_CALL_EXTERNAL;
            }
        }

        if (isSvlteSupported() && getPhoneCount() == 1 && oldMode == AudioManager.MODE_IN_CALL) {
            if (!isRoaming()) {
                newMode = AudioManager.MODE_IN_CALL_EXTERNAL;
            } else {
                newMode = AudioManager.MODE_IN_CALL;
