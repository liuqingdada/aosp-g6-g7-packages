...
//                                            + " LIMIT 1";
                    ...
                    String dateQuery = "SELECT date FROM threads WHERE _id = " + thread_id;
...
//                   "END;");
    ...
    /// M: Add for OP01 RCS feature: @{
    /// M: record group chat system messages
    private void createRCSTable(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE sys_ipmsg (" +
                "_id INTEGER PRIMARY KEY AUTOINCREMENT," +
    ...
    public static int deleteRcsThreads(SQLiteDatabase db, String[] ids) {
        Log.d(TAG, "deleteRcsThreads:" + ids.toString());
        String in = null; 
        ...
        Cursor cursor = db.query(MmsSmsProvider.TABLE_THREADS, projection, where, null, null, null, null);
        ...
        int count = db.delete(MmsSmsProvider.TABLE_THREADS, where, null);
        // maybe the below function is not needed
        removeOrphanedAddresses(db);
        for (long deletedThread : threadIds) {
            File wallpaperPath = new File(WALLPAPER_PATH);
            if (wallpaperPath.exists()) {
                String threadWallpaperName = deletedThread + ".jpeg";
                String[] oldFile = wallpaperPath.list();
